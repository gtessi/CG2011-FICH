...
*/
	// código nuevo
	// aplico una máscara de color para dibujar el icosaedro
	// en memoria pero no en pantalla
	glColorMask(0,0,0,0); // no dibuja ningún color
	glColor4fv(face_color);
	glutSolidIcosahedron();		
	glColorMask(1,1,1,1); // dibuja los colores
	
    if (!color) glDisable(GL_LIGHTING); // lineas sin material (siempre en color)

	// primero, aplica línea punteada al fondo del icosaedro	
	glDepthFunc(GL_GREATER); // define el criterio para el z-test
	glLineWidth(1); // espesor de líneas (finas)
	glColor4fv(line_color); // color de líneas
	glLineStipple(1,0x00FF); // tipo de línea punteada
	glEnable(GL_LINE_STIPPLE); // comienza a dibujar líneas punteadas
	  glutWireIcosahedron(); // dibuja las líneas del icosaedro
	glDisable(GL_LINE_STIPPLE); // deja de dibujar líneas punteadas

	// luego, aplica línea sólida al frente del icosaedro
	glDepthFunc(GL_LESS); // define el criterio para el z-test
	glLineWidth(3); // espesor de líneas (gruesas)
	glColor4fv(line_color); // color de líneas
	glutWireIcosahedron(); // dibuja las líneas del icosaedro	
