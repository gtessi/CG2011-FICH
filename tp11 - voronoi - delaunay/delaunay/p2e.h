...
public:
  ...
  // comparación
  bool eq_b2(const int*,const int r2=0) const; // circulo dado r^2
  bool eq_b2(const p2e& p,const int r2=0) const {return eq_b2(p.coord,r2);}
  ...
  p2e operator-(const p2e &p) const // resta
    {return p2e(x-p.x,y-p.y);}
  int operator%(const p2e &p) const // producto vectorial (% =precedencia *)
    {return x*p.y-y*p.x;}
  int operator*(const p2e &p) const // producto escalar
    {return x*p.x+y*p.y;}
  ...
  // distancia
  int distancia2(const p2e &p) const // distancia al cuadrado
    {return cuadrado(x-p.x)+cuadrado(y-p.y);}
  friend int distancia2(const p2e &a,const p2e &b)
  ...
  // modulo
  int mod2() const   // modulo al cuadrado
    {return cuadrado(x)+cuadrado(y);}
  friend int mod2(const p2e &p) {return p.mod2();}
  ...
  bool interseccion( // la pone en *this
      const p2e &p00, const p2e &p01, 
      const p2e &p10, const p2e &p11, 
      int params[3]); // los parametros reales son 0/2 y 1/2
  ...
  friend void c3(const p2e &, const p2e &, const p2e &, p2e &c, int &a);
  friend void c3 (const p2e **p, p2e &c, int &a)
    {c3(*(p[0]),*(p[1]),*(p[2]),c,a);}
  ...
  p2e(const p2e &p0, const p2e &p1, const p2e &p2, const int *a);
  // calcula las tres coordenadas de area
  // si el area ya viene precalculada poner true
  void fforma(const p2e&, const p2e&, const p2e&, int *, bool area_calculada=false) const;
};
