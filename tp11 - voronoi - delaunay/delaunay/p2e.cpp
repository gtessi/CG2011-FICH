...
#include "p2e.h"
  ...
  // comparación
bool p2e::eq_b2(const int* p,const int r2) const { // circulo dado r^2
  int d2=cuadrado(p[0]-x); if (d2>r2) return false;
  d2+=cuadrado(p[1]-y); if (d2>r2) return false;
  return true;
}
...
bool hay_interseccion_estricta(
  ...
  // c es (centro-p0)*a
void c3(const p2e &p0, const p2e &p1, const p2e &p2, p2e &c, int &a){
  p2e P1=p1-p0,P2=p2-p0; // origen en p0
  a=(P1%P2)<<1;
  int m2P1=P1.mod2(),m2P2=P2.mod2();
...
p2e::p2e(const p2e &p0, const p2e &p1, const p2e &p2, const int *a){// constructor
  if (!a[3]) {x=y=0; return;}
...
void p2e::fforma(const p2e &p0, const p2e &p1, const p2e &p2, int *a, bool area_calculada) const {
