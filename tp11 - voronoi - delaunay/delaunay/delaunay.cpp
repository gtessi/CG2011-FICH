...
// Calcula si un punto esta o no dentro del circulo
      ...
      //   PT^2 - 2 PT.C - C^2 <? C^2
      //   PT^2 <? 2 PT.C
  // Desgraciadamente no puedo multiplicar por aa para 
  //   trabajar con enteros porque me voy de 32 bits
  // Estamos con distancias de orden 1e3 (pantalla), 
  //   al cuadrado y encima multiplicando por el area 
  //   (distancia al cuadrado) enseguida llegamos al maximo entero
  // De todos modos se podria hacer mas eficiente comparando 
  //   antes con el bounding-box (solo deltax y deltay)

// en el circulo o el borde (<=)
bool Dtri::tiene_c(const p2e &pt) const{
  p2e PT=pt-*(p[0]);
  return (PT.mod2()<=2.0*PT[0]*cc[0]/aa+2.0*PT[1]*cc[1]/aa);
}
// interior estricto (<)
...
Dtri& delaunay::enquetriangulo(const p2e &p, int a[4], Dtri *start){
...
bool delaunay::agregapunto(p2e &p){
  ...
  int a[4], amin, imin, amed, imed, amax, imax;
...
bool delaunay::quitapunto(const p2e &p){
  ...
  while (ptris.deep>3){
    for (j=0;j<ptris.deep;j++){
      // en el swap es como que los triangulos "giran a la izquierda";
