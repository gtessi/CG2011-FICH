...
*/
  ...
  // fija las variables SE0 y E0 en el punto (0,r)
  int SE=5-2*r, // 2*(0-r)+5 = 5-2*r
	  E=3; // 2*0+3 = 3
...
//cout << x << "," << y << "\n";
	...
	// verifico para dónde me desplazo según la variable
	// de decisión h y actualizo los incrementos porque
	// son constantes, según para dónde me desplacé
	if (h>=0){ // dibuja pixel SE en el paso SEi
		//h+=((x-y)<<1)+5; // algoritmo viejo
		h+=SE; // actualizo la variable de decisión h
		SE+=4; // SEi+1
		E+=2; // Ei+1
		y--;
	}
	else { // dibuja pixel E en el paso Ei
		//h+=(x<<1)+3; // algoritmo viejo
		h+=E; // actualizo la variable de decisión h
		SE+=2; // SEi+1
		E+=2; // Ei+1
	}
	x++;
...
void Mouse_cb(int button, int state, int x, int y){
  ...
  if (button==GLUT_LEFT_BUTTON){
    if (state==GLUT_DOWN) {
      ...
      // implementado con tolerancia tol
	  // metrica de rombo
	  int difini=abs(x-xini)+abs(y-yini), // dif abs del primer punto
		  diffin=abs(x-xfin)+abs(y-yfin), // dif abs del último punto
		  tol=10; // tolerancia
	  
	  if (pdef==2){ // ya se definieron los puntos, se pueden editar
		if (difini<tol){
			edit_ini=true; // habilita el picado del primer punto
			glutMotionFunc(Motion_cb);
		}
		if (diffin<tol){
			edit_ini=false; // habilita el picado del último punto
			glutMotionFunc(Motion_cb);
		}
		return;
	  }
